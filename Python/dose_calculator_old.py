...
def Superposition(kernel_arrays,kernel_size,num_planes,voxel_lengths,voxel_info,beam_coor,eff_distance,mat_array,num_cores):
    ...
    mat_in_array = [False,False,False]
    mat_array_flat = np.array(mat_array).flatten()
    if 'w' in mat_array_flat:
        mat_in_array[WATER_IND] = True
    if 'l' in mat_array_flat:
        mat_in_array[LUNG_IND] = True
    if 'b' in mat_array_flat:
        mat_in_array[CORT_BONE_IND] = True
